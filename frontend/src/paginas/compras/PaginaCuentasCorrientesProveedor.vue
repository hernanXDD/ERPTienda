...
import { useCuentaCorrienteProveedorStore, type MovimientoConSaldoProveedor } from '../../stores/cuentaCorrienteProveedor';
import { useRegistroComprasStore } from '../../stores/registroCompras';
import { exportarPdfCuentaCorrienteProveedor } from '../../modulos/compras/exportarPdfCuentaCorrienteProveedor';
import { exportarReciboPagoCuentaCorrienteProveedorPdf } from '../../modulos/compras/impresionReciboPagoCuentaCorrienteProveedor';
...
import type { TipoMovimientoCuentaCorrienteProveedor } from '../../tipos/cuentaCorrienteProveedor';
import {
...
const puedeGestionarCuentaCorrienteProveedor = computed(() => tienePermiso('puedeGestionarCuentaCorrienteProveedor'));
...
const tipoMovimientoManual = ref<TipoMovimientoCuentaCorrienteProveedor>('cargo');
...
function estaMovimientoEnRangoFecha(m: MovimientoConSaldoProveedor): boolean {
...
const cuentasHabilitadas = computed(() =>
  [...proveedores.value]
    .filter((c) => c.habilitado && c.comprasCreditoHabilitadas)
    .sort((a, b) => a.nombre.localeCompare(b.nombre, 'es', { sensitivity: 'base' }))
);

function proveedorTieneDeudaPendiente(proveedorId: string): boolean {
  return cuentaCorrienteProveedorStore.saldoProveedorCacheado(proveedorId) > 0;
}
...
const cuentasFiltradas = computed(() => {
  ...
  return cuentasHabilitadas.value.filter((c) => {
    ...
    if (filtroDeuda.value === 'con-deuda' && !proveedorTieneDeudaPendiente(c.id)) return false;
    if (filtroDeuda.value === 'sin-deuda' && proveedorTieneDeudaPendiente(c.id)) return false;
    return true;
  });
});
...
const movimientosFiltradosModal = computed(() => {
  const id = proveedorModal.value?.id;
  if (!id) return [];
  return cuentaCorrienteProveedorStore.movimientosConSaldoProveedor(id).filter(estaMovimientoEnRangoFecha);
});

const saldoDeudorModal = computed(() => {
  const id = proveedorModal.value?.id;
  if (!id) return 0;
  return Math.max(0, cuentaCorrienteProveedorStore.saldoProveedorCacheado(id));
});

const cantidadCuentasProveedor = computed(() => cuentasHabilitadas.value.length);

const cantidadCuentasVisibles = computed(() => cuentasFiltradas.value.length);

const saldoTotalDeuda = computed(() =>
  cuentasFiltradas.value.reduce((acumulado, proveedorActual) => {
    return acumulado + cuentaCorrienteProveedorStore.saldoProveedorCacheado(proveedorActual.id);
  }, 0)
);

const limiteTotalProveedores = computed(() =>
  cuentasFiltradas.value.reduce(
    (acumulado, proveedorActual) => acumulado + proveedorActual.limiteCreditoCompras,
    0
  )
);

function etiquetaTipoMovimiento(tipo: MovimientoConSaldoProveedor['tipoMovimiento']): string {
...
function abrirMovimientosProveedor(c: Proveedor): void {
  proveedorModal.value = c;
...
function abrirFormularioPago(): void {
  if (!proveedorModal.value) return;
  cerrarCuadroMovimientoManual();
  errorFormularioPago.value = '';
  const hoy = new Date();
...
async function registrarMovimientoManual(): Promise<void> {
  ...
  try {
    await cuentaCorrienteProveedorStore.agregarMovimientoManual(
      c.id,
...
onMounted(() => {
  void (async () => {
    await Promise.all([
      ...
      registroComprasStore.asegurarCargado(),
    ...
    const ids = proveedoresStore.proveedores
      .filter((pr) => pr.habilitado && pr.comprasCreditoHabilitadas)
      .map((pr) => pr.id);
    await cuentaCorrienteProveedorStore.cargarMovimientosTodos(ids);
  })();
});

async function imprimirReciboPagoDesdeHistorial(movimiento: MovimientoConSaldoProveedor): Promise<void> {
  const c = proveedorModal.value;
...
async function imprimirCuentaProveedor(): Promise<void> {
  const c = proveedorModal.value;
  ...
  try {
    ...
    await exportarPdfCuentaCorrienteProveedor({
      proveedor: c,
...
<template>
  <section class="pg-wrap" aria-labelledby="titulo-cc">
    <div class="pg-marco">
      <header class="pg-cab">
        <div class="pg-cab-txt">
          <p class="pg-eyebrow">Compras · Cuentas corrientes</p>
          <h1 id="titulo-cc" class="pg-titulo">Cuentas corrientes proveedores</h1>
          <p class="pg-sub">{{ descripcionPagina }}</p>
        </div>
        <div class="pg-kpis" role="group" aria-label="Resumen de deuda con proveedores">
          <div class="pg-kpi">
            <span class="pg-kpi-etiq">Cuentas activas</span>
            <span class="pg-kpi-valor pg-mono">{{ cantidadCuentasProveedor }}</span>
          </div>
          <div class="pg-kpi">
            <span class="pg-kpi-etiq">Límite combinado</span>
            <span class="pg-kpi-valor pg-mono">{{ formatearMoneda(limiteTotalProveedores) }}</span>
          </div>
          <div class="pg-kpi pg-kpi--peligro">
            <span class="pg-kpi-etiq">Deuda total</span>
            <span class="pg-kpi-valor pg-mono">{{ formatearMoneda(saldoTotalDeuda) }}</span>
          </div>
        </div>
      </header>

      <div class="pg-barra" role="search" aria-label="Filtrar cuentas corrientes de proveedores">
      ...
      <p v-if="hayFiltrosListadoActivos" class="pg-resumen" role="status">
        ...
        {{ cantidadCuentasProveedor }}
        {{ cantidadCuentasProveedor === 1 ? 'cuenta' : 'cuentas' }} con crédito habilitado.
      </p>
      ...
      <section class="pg-tabla-cuerpo" aria-labelledby="cc-listado-cartera-tit">
        <header class="pg-tabla-cab">
          ...
          <span class="pg-tabla-meta pg-mono">
            ...
            {{ cantidadCuentasVisibles === 1 ? 'proveedor visible' : 'proveedores visibles' }}
          </span>
        </header>
            ...
            <button type="button" class="cc-cartera-tarjeta" @click="abrirMovimientosProveedor(c)">
              ...
              <dl class="cc-cartera-tarjeta-montos">
                <div class="cc-cartera-tarjeta-monto">
                  <dt>Límite CC</dt>
                  <dd class="pg-mono">{{ formatearMoneda(c.limiteCreditoCompras) }}</dd>
                </div>
                <div class="cc-cartera-tarjeta-monto cc-cartera-tarjeta-monto--saldo">
                  <dt>Saldo actual</dt>
                  <dd class="pg-mono">
                    {{ formatearMoneda(cuentaCorrienteProveedorStore.saldoProveedorCacheado(c.id)) }}
                  </dd>
                </div>
                <div class="cc-cartera-tarjeta-monto">
                  <dt>Disponible</dt>
        ...
        >
          <table class="pg-tabla pg-tabla--estado cc-tabla-cartera">
            <thead>
              <tr>
                <th scope="col" class="cc-col-cliente">Proveedor</th>
            ...
            <tbody>
              <tr v-for="c in cuentasFiltradas" :key="c.id">
                ...
                <td class="pg-der pg-mono cc-col-monto-lista cc-col-limite cc-col-oculta-movil">
                  {{ formatearMoneda(c.limiteCreditoCompras) }}
                </td>
                ...
                <td class="pg-acc cc-col-acciones">
                  <button
                    ...
                    @click="abrirMovimientosProveedor(c)"
                  >
              ...
              <tr v-if="cuentasFiltradas.length === 0">
                <td colspan="6" class="pg-vacio">
                  ...
                  <template v-else>
                    Ningún proveedor coincide con los filtros elegidos. Probá otro nombre o cambiá el
                    estado de deuda.
                  </template>
                </td>
              </tr>
    ...
    >
      <div v-if="proveedorModal" class="cc-modal-panel" @click.stop>
        <header class="cc-modal-cab">
          <div class="cc-modal-cab-bloque">
            ...
            <p class="cc-modal-sub">
              <span class="cc-modal-sub-nombre">{{ proveedorModal.nombre }}</span>
              <span class="cc-modal-sub-sep" aria-hidden="true">·</span>
              <span class="cc-mono">{{ proveedorModal.documento }}</span>
            </p>
          </div>
          <div class="cc-modal-kpis" role="group" aria-label="Resumen de saldos">
            <div class="pg-kpi">
              ...
              <span class="pg-kpi-valor pg-mono">{{
                formatearMoneda(cuentaCorrienteProveedorStore.saldoProveedorCacheado(proveedorModal.id))
              }}</span>
            </div>
            <div class="pg-kpi pg-kpi--acento">
              <span class="pg-kpi-etiq">Disponible para compras</span>
              <span class="pg-kpi-valor pg-mono">{{ formatearMoneda(creditoDisponible(proveedorModal)) }}</span>
            </div>
          </div>
        ...
        <div class="cc-modal-barra">
          ...
          <div class="cc-modal-toolbar">
            <button
              v-if="puedeGestionarCuentaCorrienteProveedor"
              type="button"
            ...
            <button
              ...
              aria-label="Exportar reporte PDF de la cuenta corriente del proveedor"
              @click="imprimirCuentaProveedor"
            >
            ...
            >
              ...
              <dl class="cc-mov-tarjeta-montos">
                ...
                <div class="cc-mov-tarjeta-monto cc-mov-tarjeta-monto--saldo">
                  <dt>Saldo</dt>
                  <dd class="cc-mono">{{ formatearMoneda(m.saldoTrasMovimiento) }}</dd>
                </div>
              </dl>
            ...
            <table class="cc-tabla cc-tabla-estado">
              ...
              <tbody>
                <tr v-for="m in movimientosFiltradosModal" :key="m.id">
                  ...
                  <td class="cc-der cc-mono cc-col-saldo">{{ formatearMoneda(m.saldoTrasMovimiento) }}</td>
                </tr>
    ...
    >
      <div v-if="proveedorModal" class="cc-reg-pago-panel" @click.stop>
        ...
        <div v-if="proveedorModal" class="cc-reg-pago-resumen" aria-label="Resumen de la cuenta">
          ...
          <div class="cc-reg-pago-resumen-item">
            ...
            <span class="cc-reg-pago-resumen-val cc-mono">{{
              formatearMoneda(proveedorModal.limiteCreditoCompras)
            }}</span>
          </div>
          <div class="cc-reg-pago-resumen-item cc-reg-pago-resumen-item--acento">
            ...
            <span class="cc-reg-pago-resumen-val cc-mono">{{
              formatearMoneda(creditoDisponible(proveedorModal))
            }}</span>
          </div>
        </div>
        ...
        <form class="cc-reg-pago-form" @submit.prevent="solicitarRegistroPago">
          ...
          <div role="group" aria-labelledby="cc-reg-pago-leyenda-dat" class="cc-reg-datos-recuadro">
            ...
            <div class="cc-form-grid">
              ...
              <label class="cc-inp-wrap cc-inp-span2">
                <span class="cc-inp-lab">Concepto (opcional)</span>
                <input
        ...
        <form class="cc-reg-pago-form" @submit.prevent="registrarMovimientoManual">
          ...
          >
            ...
            <p class="cc-mov-manual-ayuda">
              Usá <strong>Cargo</strong> para aumentar la deuda (facturas pendientes de carga,
              intereses, ajustes). Usá <strong>Abono</strong> para descontar (pagos fuera del flujo
              habitual, correcciones).
            </p>

            <div class="cc-form-grid">
              <div class="cc-inp-wrap cc-inp-span2">
                <span class="cc-inp-lab">Tipo de movimiento</span>
                <div class="cc-tipo-mov" role="radiogroup" aria-label="Cargo o abono">
                  <label
