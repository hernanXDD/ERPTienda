...
import {
  estilosEmisorReporteInlineCss,
  generarHtmlEmisorReporteInline,
  generarHtmlPieContactoInline,
  obtenerEmisorNegocioReporte,
} from '../reportes/emisorNegocioReporte';
...
export function abrirImpresionCuentaCorriente(datos: DatosImpresionCuentaCorriente): void {
  ...
  const bloqueEmisor = generarHtmlEmisorReporteInline(emisor, {
    tituloDocumento: 'Estado de cuenta corriente',
    generadoEl: fechaEmisionLegible,
  });
  const pieContacto = generarHtmlPieContactoInline(emisor);
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    h1 { font-size: 1.05rem; margin: 0 0 0.25rem; font-weight: 700; color: #334155; text-transform: uppercase; letter-spacing: 0.05em; }
    ${estilosEmisorReporteInlineCss}
    .muted { color: #64748b; font-size: 0.875rem; }
    .cab { margin-bottom: 1.25rem; }
...
<body>
  ...
  <header class="cab">
    ${bloqueEmisor}
    <p class="muted">Historial completo al momento de emisión.</p>
  </header>
  ...
  ${pieContacto}
  <footer class="muted">Vista imprimible interna</footer>
</body>
</html>`;
